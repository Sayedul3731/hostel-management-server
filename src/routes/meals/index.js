...
router.get("/users/:text", async (req, res) => {
  const result = await user.findOne({
    $or: [{ name: req.params.text }, { email: req.params.text }],
  });
  res.send([result]);
});
...
router.patch("/seats/:id", async (req, res) => {
  ...
  const result = await seat.findOneAndUpdate(
    { room_number: req.params.id, "seats.seat_no": seat_no },
    {
      $set: {
...
router.get("/reviews/:id", async (req, res) => {
  ...
  const result = await review
    .find({
      $or: [{ mealId: req.params.id }, { userEmail: req.params.id }],
    })
    .skip(page * size)
    .limit(size);
  res.send(result);
...
router.get("/reviews/:email", async (req, res) => {
  const page = parseInt(req.query.page);
  const size = parseInt(req.query.size);
  const result = await requestedMeal
    .find()
    .skip(page * size)
    .limit(size);
  res.send(result);
...
router.get("/upcomingMeals", async (req, res) => {
  const page = parseInt(req.query.page);
  const size = parseInt(req.query.size);
  const result = await upcomingMeal
    .find()
    .skip(page * size)
    .limit(size);
  res.send(result);
...
router.patch("/upcomingMeals/:id", async (req, res) => {
  const id = req.params.id;
  const result = await upcomingMeal.updateOne(
    { _id: id },
    {
      $inc: {
        like: 1,
